 ...
 *
 * @class    WC_LafkaCombos_Core_Compatibility
 * @version  6.3.1
 */
class WC_LafkaCombos_Core_Compatibility {
	...
	public static function is_wp_version_gt( $version ) {
		if ( ! isset( self::$is_wp_version_gt[ $version ] ) ) {
			global $wp_version;
			self::$is_wp_version_gt[ $version ] = $wp_version && version_compare( WC_LafkaCombos()->plugin_version( true, $wp_version ), $version, '>' );
		}
	...
	public static function is_wp_version_gte( $version ) {
		if ( ! isset( self::$is_wp_version_gte[ $version ] ) ) {
			global $wp_version;
			self::$is_wp_version_gte[ $version ] = $wp_version && version_compare( WC_LafkaCombos()->plugin_version( true, $wp_version ), $version, '>=' );
		}
