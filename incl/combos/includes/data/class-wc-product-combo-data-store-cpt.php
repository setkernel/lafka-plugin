...
/**
 * WC Product Combo Data Store class
 *
 * Combo data stored as Custom Post Type. For use with the WC 2.7+ CRUD API.
 *
 * @class    WC_Product_Combo_Data_Store_CPT
 * @version  6.7.8
 */
class WC_Product_Combo_Data_Store_CPT extends WC_Product_Data_Store_CPT {

	/**
	 * Data stored in meta keys, but not considered "meta" for the Combo type.
	 * @var array
	 */
	...
	/**
	 * Callback to exclude combo-specific meta data.
	 *
	 * @param  object  $meta
	 * @return bool
	 */
	...
	/**
	 * Reads all combo-specific post meta.
	 *
	...
	/**
	 * Writes all combo-specific post meta.
	 *
	 * @param  WC_Product_Combo  $product
	 * @param  boolean            $force
	 */
	protected function update_post_meta( &$product, $force = false ) {
		...
		foreach ( $props_to_update as $meta_key => $property ) {
			...
			if ( in_array( $property, array( 'combo_stock_quantity', 'combined_items_stock_status', 'min_raw_price', 'min_raw_regular_price', 'max_raw_price', 'max_raw_regular_price' ) ) ) {
				continue;
			}
	...
	public function save_stock_sync_props( &$product, $props_to_save ) {
		...
		if ( in_array( 'combined_items_stock_status', $props_to_save ) ) {

			$combined_items_stock_status = $product->get_combined_items_stock_status( 'edit' );

			if ( update_post_meta( $id, '_wc_pb_combined_items_stock_status', $combined_items_stock_status ) ) {
				...
				if ( 'instock' === $product->get_stock_status() ) {
					...
					if ( 'instock' === $combined_items_stock_status ) {
				...
				$updated_props[] = 'combined_items_stock_status';
			}
		}

		if ( in_array( 'combo_stock_quantity', $props_to_save ) ) {
			if ( update_post_meta( $id, '_wc_pb_combo_stock_quantity', $product->get_combo_stock_quantity( 'edit' ) ) ) {
				$updated_props[] = 'combo_stock_quantity';
			}
		}

		if ( in_array( 'combined_items_stock_sync_status', $props_to_save ) ) {
			// Does not trigger 'woocommerce_product_object_updated_props'.
			update_post_meta( $id, '_wc_pb_combined_items_stock_sync_status', $product->get_combined_items_stock_sync_status( 'edit' ) );
		}
	...
	protected function get_data_for_lookup_table( $id, $table ) {
		...
		if ( 'wc_product_meta_lookup' === $table ) {
			...
			$stock_status = 'outofstock' === get_post_meta( $id, '_wc_pb_combined_items_stock_status', true ) ? 'outofstock' : get_post_meta( $id, '_stock_status', true );
	...
	/**
	 * Writes combo raw price meta to the DB.
	 *
	 * @since  6.5.0
	 *
	 * @param  WC_Product_Combo  $product
	...
	public function save_raw_price_props( &$product ) {
		...
		/**
		 * 'woocommerce_combos_update_price_meta' filter.
		 *
		 * Use this to prevent combo min/max raw price meta from being updated.
		 *
		 * @param  boolean            $update
		 * @param  WC_Product_Combo  $this
		 */
		$update_raw_price_meta = apply_filters( 'woocommerce_combos_update_price_meta', true, $product );
	...
	/**
	 * Prepares the specified combo IDs for re-syncing.
	 *
	 * @param  array  $combo_ids
	 * @return void
	 */
	public function reset_combined_items_stock_status( $combo_ids = array() ) {
		...
		if ( empty( $combo_ids ) ) {
			...
			foreach ( $combo_ids as $combo_id ) {
				$cache_key = WC_Cache_Helper::get_cache_prefix( 'combined_data_items' ) . $combo_id;
				wp_cache_delete( $cache_key, 'combined_data_items' );
			}
		}
	}

	/**
	 * Deletes the combined items stock status sync meta of the specified IDs.
	 *
	 * @since  6.5.0
	 *
	 * @param  array  $ids
	 * @return void
	 */
	public function delete_combined_items_stock_sync_status( $ids ) {
	...
	/**
	 * Gets combo IDs having a combined items stock sync status.
	 *
	 * @since  6.5.0
	 *
	 * @return array
	 */
	public function get_combined_items_stock_sync_status_ids( $status ) {
	...
	/**
	 * Deletes the combined items stock status meta of the specified IDs.
	 *
	 * @param  array  $ids
	 * @return void
	 */
	public function delete_combined_items_stock_status( $ids ) {
	...
	/**
	 * Gets combo IDs having a combined items stock status.
	 *
	 * @return array
	 */
	public function get_combined_items_stock_status_ids( $status ) {

		if ( 'unsynced' === $status ) {
			return $this->get_combined_items_stock_sync_status_ids( $status );
		}
	 ...
	 * @return void
	 */
	public function preload_combined_product_data( $ids ) {

		if ( empty( $ids ) ) {
			return;
		}

		$cache_key = 'wc_combined_product_db_data_' . md5( json_encode( $ids ) );
		$data      = WC_LafkaCombos_Helpers::cache_get( $cache_key );
	...
	*/
	...
	public function save_combined_items_stock_sync_status( &$product ) {
		_deprecated_function( __METHOD__ . '()', '6.5.0', __CLASS__ . '::save_stock_sync_props()' );
		$this->save_stock_sync_props( $product, array( 'combined_items_stock_sync_status' ) );
	}
	public function save_combined_items_stock_status( &$product ) {
		_deprecated_function( __METHOD__ . '()', '6.5.0', __CLASS__ . '::save_stock_sync_props()' );
		$this->save_stock_sync_props( $product, array( 'combined_items_stock_status' ) );
	}
