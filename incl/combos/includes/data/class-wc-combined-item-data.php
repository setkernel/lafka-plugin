...
/**
 * Combined Item Data class.
 *
 * A container that represents a combined item and handles CRUD - @see WC_LafkaCombos_Install::get_schema(). Simplified version of @see WC_Data.
...
class WC_Combined_Item_Data {
	...
	public function __construct( $item = 0 ) {
		if ( $item instanceof WC_Combined_Item_Data ) {
	...
	*/
	...
	public function get_id() {
		return $this->get_combined_item_id();
	}
	...
	*/
	...
	public function set_id( $value ) {
		$this->set_combined_item_id( $value );
	}

	/**
	 * Set combined item ID.
	 *
	 * @param  int  $value
	 */
	...
	*/
	...
	private function create() {
		...
		do_action( 'woocommerce_new_combined_item', $this );
	}
	...
	public function read( $item ) {
		...
		if ( is_numeric( $item ) && ! empty( $item ) ) {
			$data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}woocommerce_lafka_combined_items WHERE combined_item_id = %d LIMIT 1;", $item ) );
		} elseif ( ! empty( $item->combined_item_id ) ) {
			$data = $item;
		} else {
			$data = false;
		}
		...
		if ( $data ) {
			$this->set_id( $data->combined_item_id );
			$this->set_product_id( $data->product_id );
			$this->set_combo_id( $data->combo_id );
	...
	public function save() {
		...
		if ( $this->get_combo_id() ) {

			// Clear WC_Combined_Item_Data objects from cache.
			$cache_key = WC_Cache_Helper::get_cache_prefix( 'combined_data_items' ) . $this->get_combo_id();
			wp_cache_delete( $cache_key, 'combined_data_items' );

			// Clear WC_Combined_Item objects from cache.
			WC_LafkaCombos_Helpers::cache_invalidate( 'wc_combined_item_' . $this->get_id() . '_' . $this->get_combo_id() );
	...
	public function delete() {
		...
		if ( $this->get_id() ) {
			global $wpdb;
			do_action( 'woocommerce_before_delete_combined_item', $this );
			$wpdb->delete( $wpdb->prefix . 'woocommerce_lafka_combined_items', array( 'combined_item_id' => $this->get_id() ) );
			$wpdb->delete( $wpdb->prefix . 'woocommerce_lafka_combined_itemmeta', array( 'combined_item_id' => $this->get_id() ) );
			do_action( 'woocommerce_delete_combined_item', $this );
		}
	}
	...
	*/
	...
	protected function read_meta_data() {
		...
		$use_cache   = ! defined( 'WC_LafkaCombos_DEBUG_OBJECT_CACHE' ) && $this->get_id() && $this->get_combo_id();
		$cache_key   = WC_Cache_Helper::get_cache_prefix( 'combined_item_meta' ) . $this->get_id();
		$cached_meta = $use_cache ? wp_cache_get( $cache_key, 'combined_item_meta' ) : false;
		...
		if ( ! $cache_loaded ) {
			...
			if ( $use_cache ) {
				wp_cache_set( $cache_key, $this->meta_data, 'combined_item_meta' );
			}
		}
	}
	...
	protected function save_meta_data() {
		...
		if ( ! empty( $raw_meta_data ) ) {
			...
			foreach ( $raw_meta_data as $meta ) {
				if ( isset( $this->meta_data[ $meta->meta_key ] ) && null !== $this->meta_data[ $meta->meta_key ] && ! in_array( $meta->meta_key, $updated_meta_keys ) ) {
					update_metadata_by_mid( 'combined_item', $meta->meta_id, $this->meta_data[ $meta->meta_key ], $meta->meta_key );
					$updated_meta_keys[] = $meta->meta_key;
				} else {
					delete_metadata_by_mid( 'combined_item', $meta->meta_id );
				}
			}
		}
		...
		foreach ( $add_meta_keys as $meta_key ) {
			if ( null !== $this->meta_data[ $meta_key ] ) {
				add_metadata( 'combined_item', $this->get_id(), $meta_key, $this->meta_data[ $meta_key ], true );
			}
		}
