...
/**
 * Product Combos DB Sync Task Runner.
 *
 * Uses https://github.com/A5hleyRich/wp-background-processing to handle tasks in the background.
 *
 * @class    WC_LafkaCombos_DB_Sync_Task_Runner
 * @version  6.7.8
 */
class WC_LafkaCombos_DB_Sync_Task_Runner extends WC_Background_Process {
	...
	public function dispatch() {
		...
		WC_LafkaCombos_Core_Compatibility::log( 'Dispatching...', 'info', 'wc_pb_db_sync_tasks' );

		$dispatched = parent::dispatch();

		if ( is_wp_error( $dispatched ) ) {
			WC_LafkaCombos_Core_Compatibility::log( sprintf( 'Unable to dispatch task runner: %s', $dispatched->get_error_message() ), 'error', 'wc_pb_db_sync_tasks' );
		}
	...
	protected function get_batch_key_prefix( $escaped = false ) {
		global $wpdb;
		return apply_filters( 'woocommerce_combos_sync_task_runner_esc_batch_query_prefix', $escaped ) ? $wpdb->esc_like( $this->identifier . '_batch_' ) : $this->identifier . '_batch_';
	}
	...
	protected function is_queue_full() {
		...
		return $count >= apply_filters( 'woocommerce_combos_sync_task_runner_queue_max_size', $this->queue_max_size );
	}
		...
		if ( ! empty( $data ) ) {
			...
			if ( ! empty( $data[ 'sync_ids' ] ) ) {
				...
				foreach ( $sync_ids as $id ) {
					...
					if ( self::time_exceeded() || self::memory_exceeded() || sizeof( $processed_ids ) >= 50 ) {
						...
						if ( ! empty( $resync_ids ) ) {
							...
							WC_LafkaCombos_Core_Compatibility::log( sprintf( 'Restarting task - processed %s IDs.', sizeof( $processed_ids ) ), 'info', 'wc_pb_db_sync_tasks' );
				...
				if ( ! empty( $delete_ids ) ) {
					...
					WC_LafkaCombos_Core_Compatibility::log( sprintf( 'Discarding invalid IDs: [%s]', implode( ', ', $delete_ids ) ), 'notice', 'wc_pb_db_sync_tasks' );

					$data_store = WC_Data_Store::load( 'product-combo' );
					$data_store->delete_combined_items_stock_sync_status( $delete_ids );
				}
	...
	protected function complete() {
		WC_LafkaCombos_Core_Compatibility::log( 'Sync complete.', 'info', 'wc_pb_db_sync_tasks' );
		parent::complete();
	}
}
