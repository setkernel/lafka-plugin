		...
		if ( in_array( $combined_product->get_type(), array( 'variable', 'variable-subscription' ) ) ) {
			...
			<div class="allowed_variations" <?php echo 'yes' === $override_variations ? '' : 'style="display:none;"'; ?>>
				...
				if ( sizeof( $variations ) < 50 ) {
					...
					?>
						<select multiple="multiple" name="combo_data[<?php echo $loop; ?>][allowed_variations][]" style="width: 95%;" data-placeholder="<?php esc_attr_e( 'Choose variations&hellip;', 'lafka-plugin' ); ?>" class="sw-select2"> 
						<?php
					...
					?>
						<select class="sw-select2-search--products" multiple="multiple" style="width: 95%;" name="combo_data[<?php echo $loop; ?>][allowed_variations][]" data-placeholder="<?php esc_attr_e( 'Search for variations&hellip;', 'lafka-plugin' ); ?>" data-action="woocommerce_search_combined_variations" data-limit="500" data-include="<?php echo esc_attr( $product_id ); ?>">
						<?php
