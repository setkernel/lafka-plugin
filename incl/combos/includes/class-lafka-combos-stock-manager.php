 ...
 *
 * @class    WC_LafkaCombos_Stock_Manager
 * @version  5.8.0
 */
class WC_LafkaCombos_Stock_Manager {
	...
	public function add_item( $product, $variation = false, $quantity = 1, $args = array() ) {
		$this->items[] = new WC_LafkaCombos_Stock_Manager_Item( $product, $variation, $quantity, $args );
	}
	 ...
	 *
	 * @param  WC_LafkaCombos_Stock_Manager  $stock
	 */
	 ...
	 * Note that in some cases, stock for a variation might be managed by the parent - this is tracked by the managed_by_id property in WC_LafkaCombos_Stock_Manager_Item.
	 *
	 * @return array
	 */
	...
	public function validate_stock( $args = array() ) {
		...
		$combo_id    = $this->product->get_id();
		$combo_title = $this->product->get_title();
		...
		foreach ( $managed_items as $managed_item_id => $managed_item ) {
			...
			try {
				...
				if ( $product_data->is_sold_individually() && $quantity > 1 ) {
					...
					$reason = sprintf( __( 'Only 1 &quot;%s&quot; may be purchased.', 'lafka-plugin' ), $product_title );

					if ( 'add-to-cart' === $context ) {
						$notice = sprintf( __( '&quot;%1$s&quot; cannot be added to your cart. %2$s', 'lafka-plugin' ), $combo_title, $reason );
					} else {
						$notice = $reason;
					}
				...
				if ( ! $product_data->is_in_stock() ) {
					...
					$reason = sprintf( __( '&quot;%s&quot; is out of stock.', 'lafka-plugin' ), $product_title );

					if ( 'add-to-cart' === $context ) {
						$notice = sprintf( __( '&quot;%1$s&quot; cannot be added to your cart. %2$s', 'lafka-plugin' ), $combo_title, $reason );
					} elseif ( 'cart' === $context ) {
						$notice = sprintf( __( '&quot;%1$s&quot; cannot be purchased. %2$s', 'lafka-plugin' ), $combo_title, $reason );
					} else {
						$notice = $reason;
					}
					...
					$reason = sprintf( __( 'There is not enough stock of &quot;%1$s&quot; (%2$s remaining).', 'lafka-plugin' ), $product_title, $product_data->get_stock_quantity() );
				...
				if ( $product_data->managing_stock() ) {
					...
					if ( isset( $quantities_in_cart[ $managed_item_id ] ) && ! $product_data->has_enough_stock( $quantities_in_cart[ $managed_item_id ] + $quantity ) ) {
						...
						$reason = sprintf( __( 'There is not enough stock of &quot;%1$s&quot; (%2$s in stock, %3$s in your cart).', 'lafka-plugin' ), $product_title, $product_data->get_stock_quantity(), $quantities_in_cart[ $managed_item_id ] );
				...
				if ( $managed_item['is_secret'] ) {
					...
					if ( 'add-to-cart' === $context ) {
						$error = sprintf( __( '&quot;%1$s&quot; cannot be added to your cart. %2$s', 'lafka-plugin' ), $combo_title, $reason );
					} elseif ( 'cart' === $context ) {
						$error = sprintf( __( '&quot;%1$s&quot; cannot be purchased. %2$s', 'lafka-plugin' ), $combo_title, $reason );
					} else {
						$error = $reason;
					}
				}
 ...
 *
 * @class    WC_LafkaCombos_Stock_Manager_Item
 * @version  5.10.0
 * @since    4.8.7
 */
class WC_LafkaCombos_Stock_Manager_Item {
