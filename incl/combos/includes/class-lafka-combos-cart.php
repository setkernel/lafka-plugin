...
/**
 * Product Combo cart functions and filters.
 *
 * @class    WC_LafkaCombos_Cart
 * @version  6.4.0
 */
class WC_LafkaCombos_Cart {

	/**
	 * Validation context for 'validate_combo_configuration'.
	...
	protected function __construct() {
		...
		// Validate combo configuration in cart.
		...
		// Add combo-specific cart item data based on posted vars.
		...
		// Load combo data from session into the cart.
		add_filter( 'woocommerce_get_cart_item_from_session', array( $this, 'get_cart_item_from_session' ), 10, 3 );

		// Refresh combo configuration fields.
		add_filter( 'woocommerce_combo_container_cart_item', array( $this, 'update_combo_container_cart_item_configuration' ), 10, 2 );
		add_filter( 'woocommerce_combined_cart_item', array( $this, 'update_combined_cart_item_configuration' ), 10, 2 );
		...
		// Sync quantities of combined items with combo quantity.
		...
		if ( ! WC_LafkaCombos_Core_Compatibility::is_wc_version_gte( '3.7' ) ) {
			add_action( 'woocommerce_before_cart_item_quantity_zero', array( $this, 'update_quantity_in_cart' ), 1 );
		}

		// Remove combined items on removing parent item.
		...
		// Additionally, combined item weights may have to be added in the container.
		add_filter( 'woocommerce_cart_shipping_packages', array( $this, 'cart_shipping_packages' ), 5 );

		// Remove recurring component of combined subscription-type products in statically-priced combos.
	...
	*/
	...
	/**
	 * Validates and adds a combo to the cart. Relies on specifying a combo configuration array with all necessary data - @see 'get_posted_combo_configuration()' for details.
	 *
	 * @param  mixed  $product_id      Id of the combo to add to the cart.
	 * @param  mixed  $quantity        Quantity of the combo.
	 * @param  array  $configuration   Combo configuration - @see 'get_posted_combo_configuration()'.
	...
	public function add_combo_to_cart( $product_id, $quantity, $configuration = array(), $cart_item_data = array() ) {
		...
		if ( $combo ) {
			...
			if ( $this->validate_combo_configuration( $combo, $quantity, $configuration ) ) {
				...
				$message = __( 'The submitted combo configuration could not be added to the cart.', 'lafka-plugin' );
	...
	/**
	 * Parses a combo configuration array to ensure that all mandatory cart item data fields are present.
	 * Can also be used to get an array with the minimum required data to fill in before calling 'add_combo_to_cart'.
	 *
	 * @param  WC_Product_Combo  $combo         Product combo whose configuration is being parsed or generated.
	 * @param  array              $configuration  Initial configuration array to parse. Leave empty to get a minimum array that you can fill with data - @see 'get_posted_combo_configuration()'.
	 * @param  boolean            $strict_mode    Set true to initialize combined product IDs to an empty string if undefined in the source array.
	 * @return array
	 */
	public function parse_combo_configuration( $combo, $configuration = array(), $strict_mode = false ) {
		...
		foreach ( $combined_items as $combined_item_id => $combined_item ) {
			...
			$item_configuration = isset( $configuration[ $combined_item_id ] ) ? $configuration[ $combined_item_id ] : array();
	...
	/**
	 * Build combo configuration array from posted data. Array example:
	 *
	 *    $config = array(
	 *        134 => array(                             // ID of combined item.
	 *            'product_id'        => 15,            // ID of combined product.
	 *            'quantity'          => 2,             // Qty of combined product, will fall back to min.
	 *            'discount'          => 50.0,          // Combined product discount, defaults to the defined value.
	 *            'title'             => 'Test',        // Combined product title, include only if overriding.
	 *            'optional_selected' => 'yes',         // If the combined item is optional, indicate if chosen or not.
	 ...
	 * @return array
	 */
	public function get_posted_combo_configuration( $product ) {
		...
		if ( is_object( $product ) && $product->is_type( 'combo' ) ) {
			...
			if ( ! empty( $combined_items ) ) {
				...
				/**
				 * 'woocommerce_product_combo_field_prefix' filter.
				 *
				 * Used to post unique combo data when posting multiple combo configurations that could include the same combo multiple times.
				 *
				 * @param  string  $prefix
				 * @param  mixed   $product_id
				 */
				$posted_field_prefix = apply_filters( 'woocommerce_product_combo_field_prefix', '', $product_id );
				...
				foreach ( $combined_items as $combined_item_id => $combined_item ) {
					...
					if ( 'variable' === $combined_product_type || 'variable-subscription' === $combined_product_type ) {
						...
						$attributes = $combined_item->product->get_attributes();
						$variations = $combined_item->get_children();
						$attr_stamp = array();
						...
						foreach ( $attributes as $attribute ) {
							...
							$combined_item_taxonomy_request_key = $posted_field_prefix . 'combo_' . $taxonomy . '_' . $combined_item_id;
							...
							// Value pre-selected?
								...
								$selected_variation_attribute_value = $combined_item->get_selected_product_variation_attribute( $attribute_name );
						...
						if ( sizeof( $variations ) > 1 ) {
							...
							$combined_item_variation_id_request_key = $posted_field_prefix . 'combo_variation_id_' . $combined_item_id;
		...
		$posted_config = $this->parse_combo_configuration( $product, $posted_config, true );

		return $posted_config;
	}

	/**
	 * Rebuilds posted form data associated with a combo configuration.
	 *
	 * @since  5.8.0
	 *
	 ...
	 * @return boolean
	 */
	public function rebuild_posted_combo_form_data( $configuration ) {
		...
		/**
		 * 'woocommerce_rebuild_posted_combo_form_data' filter.
		 *
		 * @since  5.8.0
		 *
		 ...
		 * @param  array  $configuration
		 */
		return apply_filters( 'woocommerce_rebuild_posted_combo_form_data', $form_data, $configuration );
	}

	/**
	 * Validates the selected combined items in a combo configuration.
	 *
	 * @param  mixed   $product
	 ...
	 * @return boolean
	 */
	public function validate_combo_configuration( $product, $product_quantity, $configuration, $context = '' ) {
		...
		if ( is_object( $product ) && $product->is_type( 'combo' ) ) {
			...
			try {
				...
				if ( '' === $context ) {
					...
					/**
					 * 'woocommerce_combo_validation_context' filter.
					 *
					 * @since  5.7.4
					 *
					 * @param  string             $context
					 * @param  WC_Product_Combo  $context
					 */
					$context = apply_filters( 'woocommerce_combo_validation_context', 'add-to-cart', $product );
				}
				...
				// If a stock-managed product / variation exists in the combo multiple times, its stock will be checked only once for the sum of all combined quantities.
				// The stock manager class keeps a record of stock-managed product / variation ids.
				$combined_stock = new WC_LafkaCombos_Stock_Manager( $product );

				// Grab combined items.
				$combined_items = $product->get_combined_items();

				if ( sizeof( $combined_items ) ) {

					foreach ( $combined_items as $combined_item_id => $combined_item ) {

						$combined_product_id   = $combined_item->get_product_id();
						$combined_variation_id = '';
						$combined_product_type = $combined_item->product->get_type();
						...
						if ( 'cart' === $context ) {
							...
							if ( $missing_contents ) {
								...
								$notice = sprintf( __( '&quot;%1$s&quot; cannot be purchased &ndash; some of its contents are missing from your cart.', 'lafka-plugin' ), $product_title );
						...
						if ( $item_quantity < $item_quantity_min ) {
							...
							$reason = sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be lower than %2$d.', 'lafka-plugin' ), $combined_item->get_raw_title(), $item_quantity_min );

							if ( 'add-to-cart' === $context ) {
								$notice = sprintf( __( '&quot;%1$s&quot; cannot be added to your cart. %2$s', 'lafka-plugin' ), $product_title, $reason );
							} elseif ( 'cart' === $context ) {
								$notice = sprintf( __( '&quot;%1$s&quot; cannot be purchased. %2$s', 'lafka-plugin' ), $product_title, $reason );
							} else {
								$notice = $reason;
							}
							...
							$reason = sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be higher than %2$d.', 'lafka-plugin' ), $combined_item->get_raw_title(), $item_quantity_max );
						...
						$quantity = $combined_item->is_sold_individually() && $item_quantity <= 1 ? 1 : $item_quantity * $product_quantity;
						...
						if ( false === $combined_item->is_purchasable() ) {
							...
							$reason = sprintf( __( '&quot;%s&quot; cannot be purchased.', 'lafka-plugin' ), $combined_item->get_raw_title() );
						...
						if ( 'variable' === $combined_product_type || 'variable-subscription' === $combined_product_type ) {
							...
							if ( $combined_variation ) {
								...
								$is_variation_excluded = $combined_item->has_filtered_variations() && ! in_array( $combined_variation_id, $combined_item->get_filtered_variations() );

								if ( $is_variation_excluded || $combined_variation->get_parent_id() !== absint( $combined_product_id ) || false === $combined_variation->is_purchasable() ) {

									if ( 'add-to-cart' === $context ) {
										$reason = sprintf( __( 'The chosen &quot;%s&quot; variation cannot be purchased.', 'lafka-plugin' ), $combined_item->get_raw_title() );
									} else {
										$reason = sprintf( __( 'The chosen &quot;%s&quot; variation is unavailable.', 'lafka-plugin' ), $combined_item->get_raw_title() );
									}
								...
								$combined_stock->add_item( $combined_product_id, $combined_variation, $quantity, array( 'combined_item' => $combined_item ) );
							}
							...
							if ( $combined_variation ) {
								...
								$variation_data = wc_get_product_variation_attributes( $combined_variation_id );
							...
							// Add item for validation.
							$combined_stock->add_item( $combined_product_id, false, $quantity, array( 'combined_item' => $combined_item ) );
						}

						/**
						 * Perform additional validation checks at combined item level.
						 *
						 * @param  boolean          $is_configuration_valid
						 * @param  WC_Product       $product
						 * @param  WC_Combined_Item  $combined_item
						 * @param  int              $quantity
						 * @param  mixed            $combined_variation_id
						 * @param  array            $configuration
						 */
						if ( false === apply_filters( 'woocommerce_combined_item_' . str_replace( '-', '_', $context ) . '_validation', $is_configuration_valid, $product, $combined_item, $quantity, $combined_variation_id, $configuration ) ) {
				...
				if ( $is_configuration_valid ) {
					...
					if ( ! empty( $combined_items ) && false === WC_Product_Combo::group_mode_has( $group_mode, 'parent_item' ) ) {

						$items_added = $combined_stock->get_items();

						if ( empty( $items_added ) ) {

							$reason = __( 'Please choose at least 1 item.', 'lafka-plugin' );
					...
					/**
					 * Perform additional validation checks at combo level.
					 *
					 * @param  boolean              $result
					 * @param  mixed                $product_id
					 * @param  WC_LafkaCombos_Stock_Manager  $combined_stock
					 * @param  array                $configuration
					 */
	...
	/**
	 * Analyzes combined cart items to characterize a combo.
	 *
	 * @since  5.8.0
	 *
	...
	/**
	 * When a combo is static-priced, the price of all combined items is set to 0.
	 * When the shipping mode is set to "combined", all combined items are marked as virtual when they are added to the cart.
	...
	private function set_combined_cart_item( $cart_item, $combo ) {
		...
		if ( $cart_item[ 'data' ]->needs_shipping() ) {
			 ...
			 *
			 * See WC_LafkaCombos_CP_Compatibility::add_combo_to_cart
			 */
			if ( isset( $combo->composited_cart_item ) ) {
				$combined_item->composited_cart_item = $combo->composited_cart_item;
			}
		...
		/**
		 * 'woocommerce_combined_cart_item' filter.
		 *
		 * Last chance to filter combined cart item data.
		 *
		 * @param  array              $cart_item
		 * @param  WC_Product_Combo  $combo
		 */
		return apply_filters( 'woocommerce_combined_cart_item', $cart_item, $combo );
	}

	/**
	 * Combo container price must be set equal to the base price when individually-priced items exist.
	 *
	 * @param  array              $cart_item
	 * @param  WC_Product_Combo  $combo
	 * @return array
	...
	private function set_combo_container_cart_item( $cart_item ) {
		...
		/**
		 * 'woocommerce_combo_container_cart_item' filter.
		 *
		 * Last chance to filter combo container cart item data.
		 *
		 * @param  array              $cart_item
		 * @param  WC_Product_Combo  $combo
		 */
		return apply_filters( 'woocommerce_combo_container_cart_item', $cart_item, $combo );
	}
	...
	/**
	 * Adds a combined product to the cart. Must be done without updating session data, recalculating totals or calling 'woocommerce_add_to_cart' recursively.
	 ...
	 * @return boolean
	 */
	private function combined_add_to_cart( $combo_id, $product, $quantity = 1, $variation_id = '', $variation = array(), $cart_item_data = array() ) {
		...
		/**
		 * 'woocommerce_combined_add_to_cart' action.
		 *
		 ...
		 */
		do_action( 'woocommerce_combined_add_to_cart', $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data, $combo_id );
	...
	*/
	...
	/**
	 * Check combo cart item configurations on cart load.
	 */
	...
	/**
	 * Add combo cart item data to validate.
	 *
	 * @since  6.1.3
	 *
	 * @param  array       $data
	...
	/**
	 * Validates add-to-cart for combos.
	 *
	 * @param  boolean  $add
	...
	public function validate_add_to_cart( $add, $product_id, $quantity, $variation_id = '', $variations = array(), $cart_item_data = array() ) {
		...
		/*
		 * Prevent combined items from getting validated when re-ordering after cart session data has been loaded:
		...
		if ( 'combo' === $product_type ) {
			...
			if ( is_a( $combo, 'WC_Product_Combo' ) && false === $this->validate_combo_add_to_cart( $combo, $quantity, $cart_item_data ) ) {
				$add = false;
			}
		}
	...
	/**
	 * Validates add-to-cart for combos.
	 * Basically ensures that stock for all combined products exists before attempting to add them to cart.
	 *
	 * @since  5.6.0
	 *
	 * @param  WC_Product_Combo  $combo
	 ...
	 * @return boolean
	 */
	public function validate_combo_add_to_cart( $combo, $quantity, $cart_item_data ) {

		$is_valid = true;

		/**
		 * 'woocommerce_combo_before_validation' filter.
		 *
	...
	/**
	 * Redirect to the cart when editing a combo "in-cart".
	 *
	 * @param  string  $url
	 * @return string
	 */
	...
	/**
	 * Filter the displayed notice after redirecting to the cart when editing a combo "in-cart".
	 *
	 * @param  string  $url
	 * @return string
	 */
	...
	/**
	 * Adds combo specific cart-item data.
	 * The 'stamp' var is a unique identifier for that particular combo configuration.
	 *
	 * @param  array  $cart_item_data
	 * @param  int    $product_id
	 * @return array
		...
		if ( 'combo' === $product_type ) {
			...
			if ( ! isset( $cart_item_data[ 'stamp' ] ) ) {
				...
				foreach ( $configuration as $combined_item_id => $combined_item_configuration ) {
					...
					/**
					 * 'woocommerce_combined_item_cart_item_identifier' filter.
					 *
					 * Filters the config data array - use this to add any combo-specific data that should result in unique container item ids being produced when the input data changes, such as add-ons data.
					 *
	...
	/**
	 * Adds combined items to the cart on the 'woocommerce_add_to_cart' action.
	 * The 'combined_by' var is added to each item to identify between combined and standalone instances of products.
	 ...
	 *
	 * @param  string  $combo_cart_key
	 * @param  int     $combo_id
	 * @param  int     $combo_quantity
	 * @param  int     $variation_id
	 * @param  array   $variation
	 * @param  array   $cart_item_data
	 * @return void
	 */
	public function combo_add_to_cart( $combo_cart_key, $combo_id, $combo_quantity, $variation_id, $variation, $cart_item_data ) {
		...
		if ( wc_pc_is_combo_container_cart_item( $cart_item_data ) ) {
			...
			foreach ( $cart_item_data[ 'stamp' ] as $combined_item_id => $combined_item_stamp ) {
				...
				/**
				 * 'woocommerce_combined_item_cart_data' filter.
				 *
				 ...
				 * @param  array  $cart_item_data
				 */
				$combined_item_cart_data = apply_filters( 'woocommerce_combined_item_cart_data', $combined_item_cart_data, $cart_item_data );

				/**
				 * 'woocommerce_combined_item_before_add_to_cart' action.
				 *
				 * @param  int    $product_id
				 * @param  int    $quantity
				 * @param  int    $variation_id
				 ...
				 * @param  array  $combined_item_cart_data
				 */
				do_action( 'woocommerce_combined_item_before_add_to_cart', $product_id, $quantity, $variation_id, $variations, $combined_item_cart_data );
				 ...
				 * @param  array  $combined_item_cart_data
				 */
				do_action( 'woocommerce_combined_item_after_add_to_cart', $product_id, $quantity, $variation_id, $variations, $combined_item_cart_data );
			}
		}
	}
	...
	/**
	 * Reload all combo-related session data in the cart.
	 *
	 * @param  array  $cart_item
	...
	/**
	 * Ensure any cart items marked as combined have a valid parent. If not, silently remove them.
	 *
	 * @param  WC_Cart  $cart
	 * @return void
	 */
	public function cart_loaded_from_session( $cart ) {
		...
		foreach ( $cart->cart_contents as $cart_item_key => $cart_item ) {
			...
			if ( wc_pc_maybe_is_combined_cart_item( $cart_item ) ) {
	...
	/**
	 * Keep quantities between combined products and container items in sync.
	 *
	 * @param  string   $cart_item_key
	...
	public function update_quantity_in_cart( $cart_item_key, $quantity = 0 ) {
		...
		if ( ! empty( WC()->cart->cart_contents[ $cart_item_key ] ) ) {
			...
			if ( wc_pc_is_combo_container_cart_item( WC()->cart->cart_contents[ $cart_item_key ] ) ) {
				...
				// Get combined cart items.
				$combined_cart_items = wc_pc_get_combined_cart_items( WC()->cart->cart_contents[ $cart_item_key ] );
	...
	/**
	 * Remove combined cart items with parent.
	 *
	 * @param  string   $cart_item_key
	 * @param  WC_Cart  $cart
	 * @return void
	...
	public function cart_item_remove( $cart_item_key, $cart ) {
		...
		if ( wc_pc_is_combo_container_cart_item( $cart->removed_cart_contents[ $cart_item_key ] ) ) {

			$combined_item_cart_keys = wc_pc_get_combined_cart_items( $cart->removed_cart_contents[ $cart_item_key ], $cart->cart_contents, true );

			foreach ( $combined_item_cart_keys as $combined_item_cart_key ) {

				$remove = $cart->cart_contents[ $combined_item_cart_key ];
				$cart->removed_cart_contents[ $combined_item_cart_key ] = $remove;

				/** WC core action. */
				do_action( 'woocommerce_remove_cart_item', $combined_item_cart_key, $cart );

				unset( $cart->cart_contents[ $combined_item_cart_key ] );
			}
		}
	}

	/**
	 * Restore combined cart items with parent.
	 *
	 * @param  string   $cart_item_key
	 * @param  WC_Cart  $cart
	 * @return void
	...
	public function cart_item_restore( $cart_item_key, $cart ) {
		...
		if ( wc_pc_is_combo_container_cart_item( $cart->cart_contents[ $cart_item_key ] ) ) {

			$combined_item_cart_keys = wc_pc_get_combined_cart_items( $cart->cart_contents[ $cart_item_key ], $cart->removed_cart_contents, true );

			foreach ( $combined_item_cart_keys as $combined_item_cart_key ) {

				$remove = $cart->removed_cart_contents[ $combined_item_cart_key ];
				$cart->cart_contents[ $combined_item_cart_key ] = $remove;

				/** WC core action. */
				do_action( 'woocommerce_restore_cart_item', $combined_item_cart_key, $cart );

				unset( $cart->removed_cart_contents[ $combined_item_cart_key ] );
			}
		}
	}
	...
	/**
	 * Treat combined subs as non-sub products when combined in statically-priced combos.
	 ...
	 * @deprecated
	 */
	public function get_combined_cart_item_container( $cart_item ) {
		_deprecated_function( __METHOD__ . '()', '5.0.0', 'wc_pc_get_combined_cart_item_container()' );
		return wc_pc_get_combined_cart_item_container( $cart_item );
	}
	public function is_combined_cart_item( $cart_item ) {
		_deprecated_function( __METHOD__ . '()', '5.0.0', 'wc_pc_is_combined_cart_item()' );
		return wc_pc_is_combined_cart_item( $cart_item );
	}
	public function is_combo_container_cart_item( $cart_item ) {
		_deprecated_function( __METHOD__ . '()', '5.0.0', 'wc_pc_is_combo_container_cart_item()' );
		return wc_pc_is_combo_container_cart_item( $cart_item );
	}
	...
	*/
	...
	public function order_again( $cart_item, $order_item, $order ) {
		_deprecated_function( __METHOD__ . '()', '5.8.0', 'WC_LafkaCombos_Order_Again::order_again_cart_item_data()' );
		return WC_LafkaCombos_Order_Again::order_again_cart_item_data( $cart_item, $order_item, $order );
	}
	public function coupon_is_valid_for_product( $valid, $product, $coupon, $item ) {
		_deprecated_function( __METHOD__ . '()', '5.8.0', 'WC_LafkaCombos_Coupon::coupon_is_valid_for_product()' );
		return WC_LafkaCombos_Coupon::coupon_is_valid_for_product( $valid, $product, $coupon, $item );
	}
	public function format_subtotal( $product, $subtotal ) {
		_deprecated_function( __METHOD__ . '()', '5.5.0', 'WC_LafkaCombos_Display::format_subtotal()' );
		return WC_LafkaCombos()->display->format_subtotal( $product, $subtotal );
	}
	public function cart_item_price_html( $price, $values, $cart_item_key ) {
		_deprecated_function( __METHOD__ . '()', '5.5.0', 'WC_LafkaCombos_Display::cart_item_price()' );
		return WC_LafkaCombos()->display->cart_item_price( $price, $values, $cart_item_key );
	}
	public function item_subtotal( $subtotal, $values, $cart_item_key ) {
		_deprecated_function( __METHOD__ . '()', '5.5.0', 'WC_LafkaCombos_Display::item_subtotal()' );
		return WC_LafkaCombos()->display->cart_item_subtotal( $subtotal, $values, $cart_item_key );
	}
	public function cart_item_quantity( $quantity, $cart_item_key ) {
		_deprecated_function( __METHOD__ . '()', '5.5.0', 'WC_LafkaCombos_Display::cart_item_quantity()' );
		return WC_LafkaCombos()->display->cart_item_quantity( $quantity, $cart_item_key );
	}
	public function cart_item_remove_link( $link, $cart_item_key ) {
		_deprecated_function( __METHOD__ . '()', '5.5.0', 'WC_LafkaCombos_Display::cart_item_remove_link()' );
		return WC_LafkaCombos()->display->cart_item_remove_link( $link, $cart_item_key );
	}
	public function woo_combos_validation( $add, $product_id, $product_quantity, $variation_id = '', $variations = array(), $cart_item_data = array() ) {
	...
	public function woo_combos_add_cart_item_data( $cart_item_data, $product_id ) {
	...
	public function woo_combos_add_combo_to_cart( $combo_cart_key, $combo_id, $combo_quantity, $variation_id, $variation, $cart_item_data ) {
		_deprecated_function( __METHOD__ . '()', '5.0.0', __CLASS__ . '::combo_add_to_cart()' );
		return $this->combo_add_to_cart( $combo_cart_key, $combo_id, $combo_quantity, $variation_id, $variation, $cart_item_data );
	}
	public function woo_combos_add_cart_item_filter( $cart_item, $cart_key ) {
		_deprecated_function( __METHOD__ . '()', '5.0.0', __CLASS__ . '::add_cart_item_filter()' );
		return $this->add_cart_item_filter( $cart_item, $cart_key );
	}
	public function woo_combos_get_cart_data_from_session( $cart_item, $cart_session_item, $cart_item_key ) {
	...
	public function woo_combos_cart_item_quantity( $quantity, $cart_item_key ) {
		_deprecated_function( __METHOD__ . '()', '5.0.0', __CLASS__ . '::cart_item_quantity()' );
		return $this->cart_item_quantity( $quantity, $cart_item_key );
	}
	public function woo_combos_cart_item_remove_link( $link, $cart_item_key ) {
	...
	public function woo_combos_update_quantity_in_cart( $cart_item_key, $quantity = 0 ) {
	...
	public function woo_combos_order_again( $cart_item_data, $order_item, $order ) {
		_deprecated_function( __METHOD__ . '()', '5.0.0', __CLASS__ . '::order_again()' );
		return $this->order_again( $cart_item_data, $order_item, $order );
	}
	public function woo_combos_cart_item_price_html( $price, $values, $cart_item_key ) {
