...
/**
 * Combined Item Product Container class.
 *
 * The bunded item class is a product container that initializes and holds pricing, availability and variation/attribute-related data for a combined product.
 *
...
class WC_Combined_Item {
	...
	/**
	 * Combined item settings meta are copied from the low-level data object to this array - @see WC_Combined_Item::load_data().
	 * @var array
	 */
	public $item_data = array();

	/**
	 * A reference to the combined item data object - @see WC_Combined_Item_Data.
	 * @var WP_Combined_Item_Data
	 */
	public $data = null;

	/**
	 * Product instance of the associated combined product.
	 * @var WC_Product
	 */
	public $product;

	/**
	 * Product instance of the parent Combo.
	 * @var WC_Product_Combo
	 */
	private $combo;

	/**
	 * The title of the combined item.
	 * @var string
	 */
	private $title;

	/**
	 * The short description of the combined item.
	 * @var string
	 */
	private $description;

	/**
	 * Visibility of the combined item in the single product, cart and order templates.
	 * @var array
	 */
	private $visibility;

	/**
	 * Price visibility of the combined item in the single product, cart and order templates.
	 * @var array
	 */
	private $price_visibility;

	/**
	 * Optional status of the combined item.
	 * @var string
	 */
	private $optional;

	/**
	 * Min quantity of the combined item.
	 * @var boolean
	 */
	private $quantity_min;

	/**
	 * Max quantity of the combined item.
	 * @var boolean
	 */
	private $quantity_max;

	/**
	 * Default quantity of the combined item.
	 * @var boolean
	 */
	private $quantity_default;

	/**
	 * Pricing scheme of the combined item.
	 * @var string
	 */
	private $priced_individually;

	/**
	 * Shipping scheme of the combined item.
	 * @var string
	 */
	private $shipped_individually;

	/**
	 * Combined item price & recurring price discount when the combined item is priced individually.
	 * @var double
	 */
	private $discount;

	/**
	 * Combined item sign-up price discount when the combined item is priced individually (unused).
	 * @var double
	 */
	...
	/**
	 * True if the combined product is a Name-Your-Price product.
	 * @var boolean
	 */
	private $is_nyp = false;

	/**
	 * Stock status of the combined product.
	 * @var string
	 */
	private $stock_status = null;

	/**
	 * Maximum stock available in a single combo purchase.
	...
	/**
	 * Products corresponding to the min/max (regular) price at which the combined product can be purchased. If the combined product is variable, these will contain the associated variations, otherwise they are identical to the 'product' property.
	 * @var WC_Product
	 */
	 ...
	 * __construct method.
	 *
	 * @param  mixed  $combined_item_id
	 * @param  mixed  $parent
	 */
	public function __construct( $combined_item, $parent = false ) {

		if ( is_numeric( $combined_item ) ) {
			$this->data = WC_LafkaCombos_DB::get_combined_item( absint( $combined_item ) );
		} elseif ( $combined_item instanceof WC_Combined_Item_Data ) {
			$this->data = $combined_item;
		}

		if ( is_object( $this->data ) ) {

			if ( is_object( $parent ) && $this->get_combo_id() === $parent->get_id() ) {
				$this->combo = $parent;
			}
			...
			/**
			 * 'woocommerce_before_init_combined_item' action.
			 *
			 * @param  WC_Combined_Item  $this
			 */
			do_action( 'woocommerce_before_init_combined_item', $this );

			$combined_product = wc_get_product( $this->get_product_id() );

			// if not present, item cannot be purchased.
			if ( $combined_product && $combined_product->is_type( array( 'simple', 'variable', 'subscription', 'variable-subscription' ) ) ) {

				$this->product     = $combined_product;
				$this->title       = 'yes' === $this->override_title ? $this->title : $combined_product->get_title();
				$this->description = 'yes' === $this->override_description ? $this->description : $combined_product->get_short_description();
			...
			/**
			 * 'woocommerce_after_init_combined_item' action.
			 *
			 * @param  WC_Combined_Item  $this
			 */
			do_action( 'woocommerce_after_init_combined_item', $this );
		}
	}
	...
	/**
	 * Get combined item ID.
	 * Returns the ID of the associated WC_Combined_Item_Data object - @see WC_Combined_Item_Data class and WC_LafkaCombos_Install::get_schema().
	 *
	 * @return int|null
	 */
	...
	/**
	 * Get the ID of the parent combo.
	 *
	 * @return int|null
	 */
	public function get_combo_id() {
		return is_object( $this->data ) ? $this->data->get_combo_id() : null;
	}

	/**
	 * Returns the parent.
	 *
	 * @return WC_Product_Combo|false
	 */
	public function get_combo() {
		if ( is_null( $this->combo ) ) {
			$this->combo = wc_get_product( $this->get_combo_id() );
		}
		return $this->combo;
	}

	/**
	 * Initialize combined item class props from combined item data object.
	 *
	 * @since 5.0.0
	 */
	...
	/**
	 * Keep combined item stock status in sync with associated product, taking 'min_quantity' into account.
	 *
	 * @since 5.0.0
	 */
	public function sync_stock() {
		...
		if ( in_array( $combined_product->get_type(), array( 'simple', 'subscription' ) ) ) {

			if ( false === $combined_product->is_in_stock() ) {

				$this->stock_status = 'out_of_stock';
				$this->max_stock    = 0;

			} elseif ( false === $combined_product->has_enough_stock( $quantity ) ) {

				$stock_quantity = $combined_product->managing_stock() ? $combined_product->get_stock_quantity() : '';
			...
			} elseif ( $combined_product->is_on_backorder( $quantity ) ) {

				$this->stock_status = 'on_backorder';
				$this->max_stock    = '';

			} elseif ( $combined_product->backorders_allowed() ) {
		...
		/*------------------------------*/
			...
			foreach ( $variation_ids as $variation_id ) {
				...
				if ( false === $variation->is_in_stock() ) {
					...
					$stock_quantity      = $variation->managing_stock() ? $variation->get_stock_quantity() : $combined_product->get_stock_quantity();
		...
		// Save changes if the combo exists in the DB.
		if ( ! defined( 'WC_LafkaCombos_DEBUG_STOCK_SYNC' ) && $this->get_combo_id() && ! doing_action( 'woocommerce_process_product_meta' ) ) {

			$combo = $this->get_combo();

			if ( $combo && $combo->is_type( 'combo' ) && 'combo' === $combo->get_data_store_type() && ! $combo->has_combined_data_item_changes() ) {
				$this->data->save();
			}
		}
	}
	...
	protected function sync_prices( $force = false ) {
		...
		$combined_product_id = $this->get_product_id();
		$combined_product    = $this->product;
		...
		if ( 'subscription' === $combined_product->get_type() ) {
			...
			// Sign up price.
			$signup_fee   = WC_Subscriptions_Product::get_sign_up_fee( $combined_product );
		...
		/*----------------------------------*/
			...
			if ( WC_LafkaCombos()->compatibility->is_nyp( $combined_product ) ) {

				$this->min_regular_price = $this->min_price = WC_Name_Your_Price_Helpers::get_minimum_price( $combined_product_id ) ? WC_Name_Your_Price_Helpers::get_minimum_price( $combined_product_id ) : 0;
	...
	public function is_discount_allowed_on_sale_price() {
		...
		/**
		 * 'woocommerce_combined_item_discount_from_regular' filter.
		 *
		 * Controls whether combined item discounts will always be applied on the regular price, ignoring any defined sale price.
		 *
	...
	/**
	 * Get combined product price after discount, price filters excluded.
	 *
	 * @param  mixed  $product
	 * @return mixed
	 */
	public function get_raw_price( $product = false, $context = '' ) {
		 ...
		 */
		$price = apply_filters( 'woocommerce_combined_item_raw_price', $combined_item_price, $product, $discount, $this, $context );

		return $price;
	}

	/**
	 * Get combined product regular price before discounts, price filters excluded.
	 *
	 * @param  mixed  $product
	 * @return mixed
	 */
	...
	/**
	 * Get combined item price, after discount, filters included.
	 *
	 * @since  5.0.0
	 *
	...
	/**
	 * Get combined item recurring price after discount, filters included.
	 *
	 * @since  5.0.0
	 *
	 * @param  string   $min_or_max
	...
	/**
	 * Get combined item regular price after discount, filters included.
	 *
	 * @since  5.0.0
	 *
	 * @param  string   $min_or_max
	...
	/**
	 * Min combined item price incl tax.
	 *
	 * @since  5.0.0
	 *
	 * @param  string  $min_or_max
	...
	/**
	 * Min combined item price excl tax.
	 *
	 * @since  5.0.0
	 *
	 * @param  string  $min_or_max
	...
	/**
	 * Min/max combined item (regular) price incl/excl tax.
	 *
	 * @since  5.5.0
	 *
	 * @param  array  $args
	...
	/**
	 * True if the combined item has a price of its own.
	 *
	 * @return boolean
	 */
	public function is_priced_individually() {
		...
		/**
		 * 'woocommerce_combined_item_is_priced_individually' filter.
		 *
		 * @param  boolean          $is_priced_individually
		 * @param  WC_Combined_Item  $this
		 */
		return apply_filters( 'woocommerce_combined_item_is_priced_individually', $is_priced_individually, $this );
	}

	/**
	 * True if the combined item is shipped individually.
	 *
	 * @return boolean
	 */
	public function is_shipped_individually() {
		...
		if ( ( $combo = $this->get_combo() ) && $combo->is_virtual() ) {
			$is_shipped_individually = true;
		}

		/**
		 * 'woocommerce_combined_item_is_shipped_individually' filter.
		 *
		 * @param  boolean          $is_shipped_individually
		 * @param  WC_Combined_Item  $this
		 */
		return apply_filters( 'woocommerce_combined_item_is_shipped_individually', $is_shipped_individually, $this );
	}
	...
	public function is_weight_aggregated( $product = false ) {
		...
		if ( ( $combo = $this->get_combo() ) && false === $this->is_shipped_individually() ) {
			$is_weight_aggregated = apply_filters( 'woocommerce_combined_item_has_combined_weight', $combo->get_aggregate_weight(), $product ? $product : $this->get_product(), $this->get_id(), $combo );
		}
	...
	/**
	 * Combined item purchasable status.
	 *
	 * @return boolean
	 */
	...
	/**
	 * Evaluates whether or not the combined item has enough stock to fulfill an arbitrary quantity requirement in a single combo purchase.
	 *
	...
	/**
	 * Combined item backorder status.
	 *
	 * @return boolean
	 */
	...
	/**
	 * Max stock available to the combo before it becomes unavailable.
	...
	/**
	 * Combined item sold individually status.
	 *
	 * @return boolean
	 */
	...
	/**
	 * Combined item name-your-price status.
	 *
	 * @return boolean
	 */
	...
	public function requires_input() {
		...
		} elseif ( false === $this->disable_addons() && WC_LafkaCombos()->compatibility->has_addons( $this->get_product(), true ) ) {
			$requires_input = true;
		}
	...
	public function display_product_variation_attribute_dropdown( $variation_attribute_name ) {
		...
		/**
		 * 'woocommerce_force_show_combined_variation_attribute_option_dropdown' filter.
		 *
		 * @param  boolean  $force_show
		 * @param  array    $args
		 */
		return $display_dropdown ? $display_dropdown : apply_filters( 'woocommerce_force_show_combined_variation_attribute_option_dropdown', false, $variation_attribute_name, $this );
	}
	...
	public function get_product_variations() {
		...
		if ( 'variable' === $this->product->get_type() || 'variable-subscription' === $this->product->get_type() ) {
			...
			$combined_item_variations = $this->product->get_available_variations();
			...
			foreach ( $combined_item_variations as $variation_data ) {
	 ...
	 *
	 * False if the combo has variation filters - otherwise ALL attribute options will show up in the dropdowns.
	 * If you still wish to enable ajax when using variation filters, use the 'woocommerce_combined_item_filtered_variations_disable_ajax' filter to prevent ajax from being disabled.
	 *
	 * @return boolean
	 */
	public function use_ajax_for_product_variations() {
		...
		if ( ! $this->exists() ) {
			...
			$use_ajax = false;
		} elseif ( $this->has_filtered_variations() && apply_filters( 'woocommerce_combined_item_filtered_variations_disable_ajax', true, $this ) ) {
			$use_ajax = false;
	 ...
	 * @param  WC_Product_Variable  $combined_product
	 * @return array
	 */
	public function filter_children( $children, $combined_product ) {
	...
	/**
	 * Modifies the results of get_available_variations() to implement variation filtering and combo discounts for variable products.
	 ...
	 * @return array
	 */
	public function filter_variation( $variation_data, $combined_product, $combined_variation ) {

		$combined_item_id = $this->get_id();

		// Disable if certain conditions are met...
		if ( $this->has_filtered_variations() ) {
			if ( ! in_array( $combined_variation->get_id(), $this->allowed_variations ) ) {
				return false;
			}
		}

		if ( '' === $combined_variation->get_price( 'edit' ) ) {
			return false;
		}
	...
	/**
	 * Add price filters to modify child product prices depending on the combined item pricing setup.
	 * Applied i) when displaying single-product form content, ii) when initializing Product Combos and iii) when calculating cart prices.
	 */
	public function add_price_filters() {
		WC_LafkaCombos_Product_Prices::add_price_filters( $this );
	}

	/**
	 * Remove price filters after modifying child product prices depending on the combined item pricing setup.
	 */
	public function remove_price_filters() {
		WC_LafkaCombos_Product_Prices::remove_price_filters();
	}
	...
	public function get_title() {
		/**
		 * 'woocommerce_combined_item_title' filter.
		 *
		 * @param  string           $title
		 * @param  WC_Combined_Item  $this
		 */
		return apply_filters( 'woocommerce_combined_item_title', $this->title, $this );
	}
	...
	public function get_raw_title( $strip_tags = false ) {
		...
		/**
		 * 'woocommerce_combined_item_raw_title' filter.
		 *
		 * @param  string           $title
		 * @param  WC_Combined_Item  $this
		 */
		return apply_filters( 'woocommerce_combined_item_raw_title', $strip_tags ? wp_strip_all_tags( $title ) : $title, $this );
	}
	...
	public function get_description() {
		/**
		 * 'woocommerce_combined_item_description' filter.
		 *
		 * @param  string           $title
		 * @param  WC_Combined_Item  $this
		 */
		return apply_filters( 'woocommerce_combined_item_description', wpautop( do_shortcode( wp_kses_post( $this->description ) ) ), $this );
	}
	...
	public function get_permalink() {
		/**
		 * 'woocommerce_combined_item_permalink' filter.
		 *
		 * @param  string           $permalink
		 * @param  WC_Combined_Item  $this
		 */
		return apply_filters( 'woocommerce_combined_item_permalink', $this->is_visible() && $this->product->is_visible() ? $this->product->get_permalink() : '', $this );
	}
	...
	public function get_optional_suffix() {
		return $this->is_optional() && apply_filters( 'woocommerce_combos_optional_combined_item_add_suffix', false, $this ) && $this->get_combo() && $this->get_combo()->contains( 'mandatory' ) ? apply_filters( 'woocommerce_combos_optional_combined_item_suffix', __( 'optional', 'lafka-plugin' ) ) : '';
	}
	...
	public function get_quantity( $type = 'min', $args = array(), $deprecated = false ) {
		...
		/**
		 * 'woocommerce_combined_item_quantity' filter.
		 *
		 * @param  mixed            $qty
		 * @param  WC_Combined_Item  $this
		 * @param  array            $args
		 */
		$qty = apply_filters( 'woocommerce_combined_item_quantity', $qty, $this, $args );
		...
		if ( in_array( $type, array( 'max', 'default' ) ) ) {
			...
			if ( $bound_by_stock && ! in_array( $product->get_type(), array( 'variable', 'variable-subscription' ) ) ) {
				...
				// Max product quantity can't be greater than the combined Max Quantity setting.
				if ( $qty_max > 0 ) {
			...
			if ( 'max' === $type ) {
				...
				/**
				 * 'woocommerce_combined_item_quantity_max' filter.
				 *
				 * @param  mixed            $qty_max
				 * @param  WC_Combined_Item  $this
				 * @param  array            $args
				 */
				$qty = apply_filters( 'woocommerce_combined_item_quantity_max', $qty_max, $this, $args );
				...
				/**
				 * 'woocommerce_combined_item_quantity_default' filter.
				 *
				 * @param  mixed            $qty_default
				 * @param  WC_Combined_Item  $this
				 * @param  array            $args
				 */
				$qty = apply_filters( 'woocommerce_combined_item_quantity_default', $qty_default, $this, $args );
			}
		}
	...
	public function get_discount( $context = '' ) {
		...
		if ( $this->is_priced_individually() ) {
			...
			/**
			 * 'woocommerce_combined_item_discount' filter.
			 *
			 * @param  mixed            $discount
			 * @param  WC_Combined_Item  $this
			 */
			$discount = apply_filters( 'woocommerce_combined_item_discount', $this->discount, $this, $context );
		}
	...
	public function get_sign_up_discount() {
		/**
		 * 'woocommerce_combined_item_sign_up_discount' filter.
		 *
		 * @param  mixed            $sign_up_discount
		 * @param  WC_Combined_Item  $this
		 */
		return $this->is_priced_individually() ? apply_filters( 'woocommerce_combined_item_sign_up_discount', $this->sign_up_discount, $this ) : '';
	}

	/**
	 * Checkbox state for optional combined items.
	 *
	 * @return boolean
	 */
	public function is_optional_checked() {
		...
		/**
		 * 'woocommerce_combined_item_is_optional_checked' filter.
		 *
		 * Use it to override the default 'checked' state of optional combined items.
		 *
		 * @param  boolean          $checked
		 * @param  WC_Combined_Item  $this
		 */
		$checked = apply_filters( 'woocommerce_combined_item_is_optional_checked', false, $this );
		 ...
		 */
		$posted_field_prefix = apply_filters( 'woocommerce_product_combo_field_prefix', '', $this->get_combo_id() );

		// When posting combined item data, set the checked status accordingly.
		if ( isset( $_REQUEST[ $posted_field_prefix . 'combo_quantity_' . $this->get_id() ] ) ) {
			if ( isset( $_REQUEST[ $posted_field_prefix . 'combo_selected_optional_' . $this->get_id() ] ) ) {
				$checked = true;
			} else {
				$checked = false;
			}
	...
	public function get_classes( $implode = true ) {
		...
		$classes = array( 'combined_item_' . $this->get_id(), 'combined_product', 'combined_product_summary', 'product' );
		...
		if ( ! $this->is_visible() ) {
			$classes[] = 'combined_item_hidden';
		}

		if ( $this->is_optional() ) {
			$classes[] = 'combined_item_optional';
		}

		/**
		 * 'woocommerce_combined_item_classes' filter.
		 *
		 * @param  array            $classes
		 * @param  WC_Combined_Item  $this
		 */
		$classes = apply_filters( 'woocommerce_combined_item_classes', $classes, $this );

		return $implode ? implode( ' ', $classes ) : $classes;
	}

	/**
	 * Get combined item stock html.
	 *
	 * @since  5.2.0
	 *
	 * @param  WC_Product|false  $product
	...
	public function get_availability_html( $product = false ) {
		...
		/**
		 * 'woocommerce_get_combined_item_stock_html' filter.
		 *
		 * Combined items availability html that takes min_quantity into account.
		 *
		 * @param  string           $availability_html
		 * @param  array            $availability
		 * @param  WC_Combined_Item  $this
		 */
		return apply_filters( 'woocommerce_get_combined_item_stock_html', $availability_html, $availability, $this, $product );
	}

	/**
	 * Combined product availability that takes min_quantity > 1 into account.
	 *
	 * @param  WC_Product|false  $product
	 * @return array
	 */
	public function get_availability( $product = false ) {
		...
		if ( ! $this->is_in_stock() || ( $product->is_type( 'variation' ) && ! $product->has_enough_stock( $quantity_min ) ) ) {
			...
			if ( $product->is_in_stock() ) {
				...
				$availability_text   = __( 'Insufficient stock', 'lafka-plugin' );
		...
		/**
		 * 'woocommerce_get_combined_item_availability_class' filter.
		 *
		 * @param  string           $availability_class
		 * @param  WC_Combined_Item  $this
		 * @param  WC_Product       $product
		 */
		$availability_class = apply_filters( 'woocommerce_get_combined_item_availability_class', $availability_class, $this, $product );

		/**
		 * 'woocommerce_get_combined_item_availability_text' filter.
		 *
		 * @param  string           $availability_text
		 * @param  WC_Combined_Item  $this
		 * @param  WC_Product       $product
		 */
		$availability_text = apply_filters( 'woocommerce_get_combined_item_availability_text', $availability_text, $this, $product );

		/**
		 * 'woocommerce_get_combined_item_availability' filter.
		 *
		 * Combined items availability needs to take min_quantity into account, hence the filter name change.
		 *
		 * @param  array            $availability
		 * @param  WC_Combined_Item  $this
		 * @param  WC_Product       $product
	...
	public function get_up_front_subscription_price( $recurring_price, $sign_up_fee, $product = false ) {
		...
		if ( WC_LafkaCombos()->compatibility->is_subscription( $product ) ) {
		...
		return round( $price, WC_LafkaCombos_Product_Prices::get_extended_price_precision() );
	}

	/**
	 * Builds a list of product attributes for a combined item.
	 *
	 * @since  5.10.1
	 *
	 * @return array
	 */
	public function get_combined_item_display_attribute_args() {
		...
		/**
		 * 'woocommerce_combo_show_combined_product_physical_props' filter.
		 * Whether to display the combined item's physical props.
		 *
		 * @since  5.8.0
		 *
		 * @param  boolean             $display_dimensions
		 * @param  WC_Product_Combos  $product
		 * @param  WC_Combined_Item     $combined_item
		 */
		$display_dimensions = apply_filters( 'woocommerce_combo_show_combined_product_physical_props', $display_dimensions, $product, $this );
		...
		foreach ( $attributes as $attribute ) {
			...
			if ( ! empty( $combined_item_variation_data ) ) {
	...
	/**
	 * Filters combined product attributes, hiding attributes that correspond to filtered-out variations. Useful up to WC 3.5 only.
	 *
	 * @param  string  $output
	 ...
	 * @return string
	 */
	public function filter_combined_item_attribute( $output, $attribute, $values ) {
	 ...
	 * @return string
	 */
	public function get_combined_item_thumbnail_size() {
		return apply_filters( 'combined_product_large_thumbnail_size', WC_LafkaCombos_Core_Compatibility::is_wc_version_gte( '3.3' ) ? 'woocommerce_thumbnail' : 'shop_catalog', $this );
	}
	...
	*/
	...
	public function get_combined_item_price_including_tax( $min_or_max = 'min', $qty = 1 ) {
	...
	public function get_combined_item_price_excluding_tax( $min_or_max = 'min', $qty = 1 ) {
