...
/**
 * Product Combo Helper Functions.
 *
 * @class    WC_LafkaCombos_Helpers
 * @version  6.5.0
 */
class WC_LafkaCombos_Helpers {
	...
	public static function format_product_title( $title, $sku = '', $meta = '', $paren = false ) {
		...
		if ( $sku && $meta ) {
			if ( $paren ) {
				$title = sprintf( _x( '%1$s &ndash; %2$s (%3$s)', 'product title followed by meta and sku in parenthesis', 'lafka-plugin' ), $title, $meta, $sku );
			} else {
				$title = sprintf( _x( '%1$s &ndash; %2$s &ndash; %3$s', 'sku followed by product title and meta', 'lafka-plugin' ), $sku, $title, $meta );
			}
		} elseif ( $sku ) {
			if ( $paren ) {
				$title = sprintf( _x( '%1$s (%2$s)', 'product title followed by sku in parenthesis', 'lafka-plugin' ), $title, $sku );
			} else {
				$title = sprintf( _x( '%1$s &ndash; %2$s', 'sku followed by product title', 'lafka-plugin' ), $sku, $title );
			}
		} elseif ( $meta ) {
			if ( $paren ) {
				$title = sprintf( _x( '%1$s (%2$s)', 'product title followed by meta in parenthesis', 'lafka-plugin' ), $title, $meta );
			} else {
				$title = sprintf( _x( '%1$s &ndash; %2$s', 'product title followed by meta', 'lafka-plugin' ), $title, $meta );
			}
		}
	...
	public static function format_product_shop_title( $title, $qty = '', $price = '', $suffix = '' ) {
		...
		if ( $qty ) {
			$quantity_string = sprintf( _x( ' &times; %s', 'qty string', 'lafka-plugin' ), $qty );
		}

		if ( $price ) {
			$price_string = sprintf( _x( ' &ndash; %s', 'price suffix', 'lafka-plugin' ), $price );
		}

		if ( $suffix ) {
			$suffix_string = sprintf( _x( ' &ndash; %s', 'suffix', 'lafka-plugin' ), $suffix );
		}

		$title_string = sprintf( _x( '%1$s%2$s%3$s%4$s', 'title, quantity, price, suffix', 'lafka-plugin' ), $title, $quantity_string, $price_string, $suffix_string );
	...
	public static function format_list_of_items( $items ) {
		...
		foreach ( $items as $key => $item ) {
			if ( $count === 1 || $loop === 1 ) {
				...
				$item_string = sprintf( _x( '%1$s and %2$s', 'string list item last separator', 'lafka-plugin' ), $item_string, $item );
			} else {
