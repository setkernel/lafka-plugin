 ...
 *
 * @class    WC_LafkaCombos_Product_Prices
 * @version  6.7.7
 */
class WC_LafkaCombos_Product_Prices {
	...
	/**
	 * Combined items whose prices are currently being filtered -- all states.
	 *
	 * @var WC_Combined_Item
	 */
	private static $combined_item_pre;

	/**
	 * Combined item whose prices are currently being filtered.
	 *
	 * @var WC_Combined_Item
	 */
	public static $combined_item;
	...
	public static function init() {
		...
		if ( 'filters' === self::get_combined_cart_item_discount_method() ) {
	...
	*/
	...
	/**
	 * A non-strict way to tell if a product's prices are being altered due to the presence of a parent "combo".
	 *
	 * @since  6.0.0
	 *
	 ...
	 * @return boolean
	 */
	public static function is_combined_pricing_context( $product, $context = 'any' ) {
	 ...
	 */
	public static function get_combined_cart_item_discount_method() {
		/**
		 * 'woocommerce_combined_cart_item_discount_method' filter.
		 *
		 * @since  6.0.0
		 *
		 * @param  string  $method  Method to use for calculating cart item discounts. Values: 'filters' | 'props'.
		 */
		$discount_method = apply_filters( 'woocommerce_combined_cart_item_discount_method', 'filters' );
	...
	public static function get_extended_price_precision( $decimals = null ) {
		...
		// Prevent infinite loops through 'wc_pc_price_num_decimals'.
	...
	/**
	 * Discounted combined item price precision. Defaults to the price display precision, a.k.a. wc_get_price_decimals.
	 *
	 * @since  5.7.8
	 *
	 * @return int
	...
	public static function get_discounted_price_precision() {
		return wc_pc_price_num_decimals( 'extended' );
	}
	 ...
	 */
	public static function add_price_filters( $combined_item ) {
		...
		if ( $add_filters ) {
			...
			/**
			 * 'woocommerce_combined_product_price_filters_added' hook.
			 *
			 * @param  WC_Combined_Item  $combined_item
			 */
			do_action( 'woocommerce_combined_product_price_filters_added', $combined_item );
		}
	}
	...
	public static function remove_price_filters() {
		...
		array_pop( self::$combined_item_pre );

		self::$combined_item = ! empty( self::$combined_item_pre ) && is_array( self::$combined_item_pre ) ? end( self::$combined_item_pre ) : null;

		if ( $combined_item && empty( self::$combined_item ) ) {
			...
			/**
			 * 'woocommerce_combined_product_price_filters_removed' hook.
			 *
			 * @param  WC_Combined_Item  $combined_item
			 */
			do_action( 'woocommerce_combined_product_price_filters_removed', $combined_item );
		}
	}
	...
	*/
	...
	public static function filter_variation_prices_hash( $hash, $product ) {
		...
		if ( $combined_item ) {
			...
			$discount                = $combined_item->get_discount();
			$has_filtered_variations = $product->is_type( 'variable' ) && $combined_item->has_filtered_variations();

			if ( $has_filtered_variations || ! empty( $discount ) ) {
				$hash[] = $combined_item->data->get_id();
			}
		}
	...
	public static function filter_children( $children, $product ) {
		...
		if ( $combined_item ) {
			...
			if ( $combined_item->has_filtered_variations() ) {

				$filtered_children = array();

				foreach ( $children as $variation_id ) {
					// Remove if filtered.
					if ( in_array( $variation_id, $combined_item->get_filtered_variations() ) ) {
	...
	/**
	 * Filter get_variation_prices() calls for combined products to include discounts.
	 *
	...
	public static function filter_get_variation_prices( $prices_array, $product ) {
		...
		if ( $combined_item ) {
			...
			if ( $priced_per_product && ! empty( $discount ) && false === $combined_item->is_discount_allowed_on_sale_price() ) {
	...
	public static function filter_show_variation_price( $show, $product, $variation ) {
		...
		if ( $combined_item ) {
			...
			if ( $combined_item->is_priced_individually() && $combined_item->is_price_visible( 'product' ) ) {
				$show = true;
				// If the product is optional and all prices are equal, then the prices is already displayed in "Add for $XXX".
				if ( $combined_item->is_optional() && $prices_equal ) {
					$show = false;
				}
			}
		}
	...
	/**
	 * Filter get_price() calls for combined products to include discounts.
	 *
	 * @param  double      $price
	 * @param  WC_Product  $product
	 * @param  string      $context
	...
	public static function filter_get_price( $price, $product, $context = '' ) {
		...
		if ( $combined_item && ( $combined_item instanceof WC_Combined_Item ) ) {
			...
			if ( $discount = $combined_item->get_discount( $context ) ) {

				$offset_price     = ! empty( $product->combined_price_offset ) ? $product->combined_price_offset : false;
				$offset_price_pct = ! empty( $product->combined_price_offset_pct ) && is_array( $product->combined_price_offset_pct ) ? $product->combined_price_offset_pct : false;
			...
			$product->combined_item_price = $price;

			/** Documented in 'WC_Combined_Item::get_raw_price()'. */
			$price = apply_filters( 'woocommerce_combined_item_price', $price, $product, $discount, $combined_item );
		}
	...
	/**
	 * Filter get_regular_price() calls for combined products to include discounts.
	 *
	 * @param  double      $price
	 * @param  WC_Product  $product
	 * @return double
	...
	/**
	 * Filter get_sale_price() calls for combined products to include discounts.
	 *
	 * @param  double      $price
	 * @param  WC_Product  $product
	 * @param  string      $context
	...
	public static function filter_get_sale_price( $sale_price, $product, $context = '' ) {
		...
		if ( $combined_item && ( $combined_item instanceof WC_Combined_Item ) ) {
			...
			if ( $discount = $combined_item->get_discount( $context ) ) {
				...
				if ( '' === $sale_price || false === $combined_item->is_discount_allowed_on_sale_price() ) {
					$regular_price = $product->get_regular_price();
				} else {
			...
			/** Documented in 'WC_Combined_Item::get_raw_price()'. */
			$sale_price = apply_filters( 'woocommerce_combined_item_price', $sale_price, $product, $discount, $combined_item );
		}
	...
	/**
	 * Filter get_price() calls for combined cart items to include discounts.
	 *
	 * @since  6.0.0
	 *
	 * @param  double      $price
	...
	public static function filter_get_price_cart( $price, $product ) {
		return self::is_combined_pricing_context( $product, 'cart' ) ? self::filter_get_price( $price, $product, 'cart' ) : $price;
	}

	/**
	 * Filter get_sale_price() calls for combined cart items to include discounts.
	 *
	 * @since  6.0.0
	 *
	 * @param  double      $price
	...
	public static function filter_get_sale_price_cart( $price, $product ) {
		return self::is_combined_pricing_context( $product, 'cart' ) ? self::filter_get_sale_price( $price, $product, 'cart' ) : $price;
	}

	/**
	 * Filter get_regular_price() calls for combined cart items.
	 *
	 * @since  6.1.4
	 *
	 * @param  double      $price
	...
	public static function filter_get_regular_price_cart( $price, $product ) {
		return self::is_combined_pricing_context( $product, 'cart' ) ? self::filter_get_regular_price( $price, $product, 'cart' ) : $price;
	}
	...
	/**
	 * Filter the html price string of combined items to show the correct price with discount and tax - needs to be hidden when the combined item is priced individually.
	 *
	 * @param  string      $price_html
	 * @param  WC_Product  $product
	 * @return string
	...
	public static function filter_get_price_html( $price_html, $product ) {
		...
		if ( $combined_item ) {
			...
			if ( ! $combined_item->is_price_visible( 'product' ) ) {
				return '';
			}

			$quantity = $combined_item->get_quantity( 'max' );

			/**
			 * 'woocommerce_combined_item_price_html' filter.
			 *
			 * @param  string           $price_html
			 * @param  WC_Combined_Item  $combined_item
			 */
			$price_html = apply_filters( 'woocommerce_combined_item_price_html', '' === $quantity || $quantity > 1 ? sprintf( __( '%1$s <span class="combined_item_price_quantity">each</span>', 'lafka-plugin' ), $price_html, $quantity ) : $price_html, $price_html, $combined_item );
		}
	...
	*/
	...
	/**
	 * Calculates combined product prices incl. or excl. tax depending on the 'woocommerce_tax_display_shop' setting.
	 *
	 * @deprecated  5.5.0
	 */
...
WC_LafkaCombos_Product_Prices::init();
