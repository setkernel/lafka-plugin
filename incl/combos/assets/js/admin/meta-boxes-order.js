/* global wc_combos_admin_order_params */
...
jQuery( function( $ ) {
		...
		functions    = {
			...
			handle_events: function() {
				...
				$order_items
					...
					.on( 'click', 'button.configure_combo', { action: 'configure' }, this.clicked_edit_button )
					.on( 'click', 'button.edit_combo', { action: 'edit' }, this.clicked_edit_button );

				$( document.body )

					.on( 'click', 'input.combined_product_checkbox', this.toggle_optional_item );
			},
			...
			toggle_optional_item: function( event ) {
				...
				var $input     = $( this ),
					...
					$content   = $input.closest( '.details' ).find( '.combined_item_cart_content' );
			...
			clicked_edit_button: function( event ) {
				...
				view = new WCPBBackboneModal( {
					target: 'wc-modal-edit-combo',
					string: {
						action: 'configure' === event.data.action ? wc_combos_admin_order_params.i18n_configure : wc_combos_admin_order_params.i18n_edit,
						item_id: item_id
					}
				} );
			...
			clicked_done_button: function( event ) {
				...
				var data = $.extend( {}, functions.get_taxable_address(), {
					action:    'woocommerce_edit_combo_in_order',
					item_id:   view._string.item_id,
					fields:    view.$el.find( 'input, select, textarea' ).serialize(),
					dataType:  'json',
					order_id:  woocommerce_admin_meta_boxes.post_id,
					security:  wc_combos_admin_order_params.edit_combo_nonce
				} );
				...
				$.post( woocommerce_admin_meta_boxes.ajax_url, data, function( response ) {
					...
					if ( response.result && 'success' === response.result ) {
						...
						if ( 'yes' === wc_combos_admin_order_params.is_wc_version_gte_3_4 ) {
						...
						if ( 'yes' === wc_combos_admin_order_params.is_wc_version_gte_3_6 ) {
							// Update notes.
						...
						window.alert( response.error ? response.error : wc_combos_admin_order_params.i18n_validation_error );
						functions.unblock( view.$el.find( '.wc-backbone-modal-content' ) );
					}
			...
			populate_form: function() {
				...
				var data = {
					action:    'woocommerce_configure_combo_order_item',
					item_id:   view._string.item_id,
					dataType:  'json',
					order_id:  woocommerce_admin_meta_boxes.post_id,
					security:  wc_combos_admin_order_params.edit_combo_nonce
				...
				$.post( woocommerce_admin_meta_boxes.ajax_url, data, function( response ) {
					...
					if ( response.result && 'success' === response.result ) {
						...
						window.alert( wc_combos_admin_order_params.i18n_form_error );
						functions.unblock( view.$el.find( '.wc-backbone-modal-content' ) );
	 ...
	 */
	if ( 'no' === wc_combos_admin_order_params.is_wc_version_gte_3_4 ) {
