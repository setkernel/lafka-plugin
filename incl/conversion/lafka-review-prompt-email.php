...
require_once __DIR__ . '/lafka-email-unsubscribe.php';
...
if ( ! function_exists( 'lafka_review_email_should_skip_order' ) ) {
	...
	function lafka_review_email_should_skip_order( $order ): bool {
		...
		// Per-recipient opt-out (CAN-SPAM/GDPR): covers guest checkout, where
		// there is no WP user and therefore no user-meta opt-out flag to read.
		if ( function_exists( 'lafka_unsub_is_opted_out' ) && lafka_unsub_is_opted_out( $email ) ) {
			return true;
		}
...
if ( ! function_exists( 'lafka_review_email_render_body' ) ) {
	...
	function lafka_review_email_render_body( $order, $email_instance = null ): string {
		...
		<p style="margin:32px 0 0 0;font-size:12px;line-height:1.5;color:#999;text-align:center;">
			...
			<?php
			// Email-keyed unsubscribe so guest-checkout customers (no WP user)
			// also get a working opt-out link; falls back to the legacy
			// user-id link only if the shared helper is unavailable.
			$unsub = function_exists( 'lafka_unsub_url' )
				? lafka_unsub_url( $billing_email )
				: lafka_review_email_unsubscribe_url( $user_id );
			if ( '' !== $unsub ) :
				?>
				<br>
			...
			<?php endif; ?>
			<?php
			$review_postal_addr = function_exists( 'lafka_unsub_postal_address' ) ? lafka_unsub_postal_address() : '';
			if ( '' !== $review_postal_addr ) :
				?>
				<br>
				<span><?php echo esc_html( $review_postal_addr ); ?></span>
			<?php endif; ?>
		</p>
		<?php
