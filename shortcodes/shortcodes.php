...
if ( ! function_exists( 'lafka_counter_shortcode' ) ) {
	...
	function lafka_counter_shortcode( $atts ) {
		...
		<div class="lafka-counter-shortcode">
			<div class="lafka-counter-content  lafka-counter-<?php echo esc_attr( $counter_style ); ?> <?php echo sanitize_html_class( $counter_alignment ); ?>" 
			<?php
			if ( $text_color ) :
				?>
...
if ( ! function_exists( 'lafka_typed_shortcode' ) ) {
	...
	function lafka_typed_shortcode( $atts ) {
		...
		if ( $static_text_color ) :
			?>
			style="color:<?php echo esc_html( $static_text_color ); ?>" <?php endif; ?> >
			<?php echo esc_html( $txt_before_typed ); ?>
			<span id="<?php echo esc_attr( $unique_id ); ?>" class="lafka-typed"  
			<?php
			if ( $typed_text_color ) :
				?>
				style="color:<?php echo esc_html( $typed_text_color ); ?>" <?php endif; ?>></span>
				...
				if ( $enable_sortable == 'yes' ) :
					...
					if ( $taxonomies ) {
						...
						if ( is_array( $taxonomies_arr ) && ! empty( $taxonomies_arr ) ) {
							$foodmenu_categories = get_terms(
								array(
									'taxonomy'         => 'lafka_foodmenu_category',
									'term_taxonomy_id' => $taxonomies_arr,
								)
							);
						}
					} else {
		...
		if ( $map_latitude && $map_longitude && ! is_search() ) {
			...
			wp_add_inline_script(
                ...
                window.addEventListener('DOMContentLoaded', () => {
                    google.maps.event.addDomListener(window, 'load', initialize('" . esc_js( $location_title ? $location_title : esc_html__( 'Our Location', 'lafka-plugin' ) ) . "', '" . esc_js( $map_latitude ) . "', '" . esc_js( $map_longitude ) . "', '" . esc_url( plugins_url( 'assets/image/google_maps/', __DIR__ ) ) . "', '" . esc_js( $map_canvas_unique_id ) . "'));
                });
	        "
			);
			...
			<?php if ( $price && $period ) : ?>
				<div class="lafka-pricing-table-price">
					<?php if ( $price ) : ?>
						...
						if ( $currency_symbol ) :
							?>
							<sub><?php echo esc_html( $currency_symbol ); ?></sub><?php endif; ?><?php echo esc_html( $price ); ?>
							<?php
							if ( $price_coins ) :
								?>
							<?php echo '<sup>.' . esc_html( $price_coins ) . '</sup>'; ?><?php endif; ?>
					<?php endif; ?>
					<?php if ( $period ) : ?>
